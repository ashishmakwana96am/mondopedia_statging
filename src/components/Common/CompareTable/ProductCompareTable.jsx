...
const ProductCompareTable = React.memo(
  ({ products, categoryAttributes, slug, productPhaseData }) => {
    ...
    const className = winPos
      ...
      : "nonSticky";
      const checkDoesCategoryToShow = (category, categoryName) => {
        const categoryList = category.attributes
          ?.sort((a, b) => a.position - b.position)
          .slice(0, pagination[category.name] || initialNoOfCategories);
              ...
              .map((category, categoryIndex) => {
                return (
                  <Fragment key={categoryIndex}>
                    ...
                    {category.attributes
                      ?.sort((a, b) => a.position - b.position).map((catAttribute, catAttributeIndex) => {
                        return (
                          <tr key={catAttributeIndex} className="" style={{display:`${checkDoesCategoryToShow(category , catAttribute.name) ? 'table-row' : 'none'}`}}>
                            <th className="sub-inner-padding">
                              <div className="tooltip-title">
                                {catAttribute.name}
